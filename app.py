...
def run_episode() -> Tuple[str, str]:
    checkpoint_path = os.getenv("CRISISGRID_CHECKPOINT_PATH", "")
    if not checkpoint_path:
        return (
            "Missing CRISISGRID_CHECKPOINT_PATH env var.",
            "Set it to your LoRA adapter directory path in the Space settings.",
        )

    global _MODEL, _TOKENIZER
    if _MODEL is None or _TOKENIZER is None:
        _MODEL, _TOKENIZER = load_model_and_tokenizer(checkpoint_path)
